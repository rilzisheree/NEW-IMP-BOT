...
export default {
  data: new SlashCommandBuilder()
    .setName('logvalor')
    .setDescription('See all valor changes in the last 24 hours. (LT only)')
    .addUserOption(opt =>
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        logs.map(l => {
          ...
          return `${emoji[l.action] ?? '🔄'} <@${l.userId}> **${amt}** by <@${l.executorId}> ${time}${note}`;
        }).join('\n')
      )
      .setFooter({ text: `Up to 25 most recent entries${filterUser ? ` for ${filterUser.username}` : ''}` })
      .setTimestamp();
